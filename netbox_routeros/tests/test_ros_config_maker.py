...
from netbox_routeros.ros_config_maker import (
    _context_prefixes,
    get_interface,
    combine_prefixes,
    get_loopback,
...
class CombinePrefixesTestCase(TestCaseMixin, TestCase):
    ...
    def test_list(self):
        self.assertEqual(
            combine_prefixes([self.prefix1, self.prefix2, self.prefix3]),
            [IPNetwork("10.0.0.0/23"), IPNetwork("10.0.5.0/24")],
        )

    def test_queryset(self):
        self.assertEqual(
            combine_prefixes(Prefix.objects.all()),
            [IPNetwork("10.0.0.0/23"), IPNetwork("10.0.5.0/24")],
        )
    ...
    def test_only_combined(self):
        self.assertEqual(
            combine_prefixes(
                [self.prefix1, self.prefix2, self.prefix3], only_combined=True
            ),
            [IPNetwork("10.0.0.0/23")],
        )
...
class GetInterfaceTestCase(TestCaseMixin, TestCase):
    def setUp(self):
        ...
        self.ether1 = self.data.interface(
            device=self.device, ip_address=["10.0.0.1/24"]
        )
        self.ether2 = self.data.interface(
            device=self.device, ip_address=["10.5.0.1/24"], vlan=self.vlan1
        )
    ...
    def test_from_ip_netaddr_network(self):
        self.assertEqual(
            get_interface(self.device, IPNetwork("10.5.0.0/24")), self.ether2
        )
...
class GetVlanInterfaceTestCase(TestCaseMixin, TestCase):
    def setUp(self):
        ...
        self.ether2 = self.data.interface(
            device=self.device, ip_address=["10.5.0.1/24"]
        )
